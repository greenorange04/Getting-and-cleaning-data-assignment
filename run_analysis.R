...
feat_test <- tbl_df(feat_test)
act_test <- tbl_df(act_test)
...
actnames_test <- tbl_df(actnames_test)
...
#names(join) <- c("activity", "subject", features$V2)
...
sum_dataset2 <- summarize(grouped_dataset, avg_mean = mean(mean), avg_std = mean(std))
